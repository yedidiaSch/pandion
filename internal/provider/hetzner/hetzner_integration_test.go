...
// Opt-in real-cloud test. Requires BOTH PANDION_IT=1 and HCLOUD_TOKEN so it can
...
func TestHetzner_Integration_CreateListDestroy(t *testing.T) {
	if os.Getenv("PANDION_IT") != "1" {
		t.Skip("set PANDION_IT=1 and HCLOUD_TOKEN to run the real-cloud integration test")
	}
	...
	clusterID := "pandion-it-" + time.Now().Format("20060102-150405")
