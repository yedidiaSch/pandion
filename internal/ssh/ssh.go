...
// rather than disabling verification (S1/F4).
func RunWithRetry(ctx context.Context, addr, user string, signer gossh.Signer, pinned gossh.PublicKey, cmd string, every time.Duration) (string, error) {
	var lastErr error
	for {
		select {
		case <-ctx.Done():
