...
- Served at: `https://yedidiaSch.github.io/pandion/`
...
- `gpg.key` — the public signing key (also committed here as
  `pandion-packages.gpg.key`)
- `pandion.repo` — a ready-to-drop `/etc/yum.repos.d/` file

## Signing key

- **Fingerprint:** `8A19045A2466ED368AA23868988E9FA033620E2F`
- Identity: `Pandion Packages <127786072+yedidiaSch@users.noreply.github.com>`
...
- To **rotate**: generate a new key, replace the secret and
  `pandion-packages.gpg.key`, and re-run the workflow.
...
After that, `sudo apt install pandion` / `sudo dnf install pandion` work per the
top-level README.
