...
| **G1** | **Lambda Labs provider** (Tier A): `CreateServer` with GPU SKU + CUDA-native image, `Pricer`, `GPUProvider`. End-to-end `up --gpu` on a real A100 with overlay + `lockdown` + `reap`. | Real cloud e2e; node reachable only over overlay |
| **G2** | **GPU-aware idle dead-man** (§4) + teardown **receipt** (§6.2) + `ls` GPU/cost columns (§6.1). | A headless training job keeps the node alive; an idle node reaps within TTL |
